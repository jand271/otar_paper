...
The security of the authenticating ciphertext assumes that (1) the Provider exclusively holds certain identifying information secret and (2) no efficient algorithms exist that can generate the authenticating ciphertext without the identifying information held secret.
If such an efficient algorithm exists, the scheme is called broken.
...
Herein, wherever we specify ECDSA, we could use another similar asymmetric cryptographic algorithm (e.g., EC-Schnorr); however, we will specify ECDSA without losing generality for concreteness.
...
TESLA uses a delayed-release mechanism to authenticated data with less authenticating ciphertext than that required by ECDSA; however, it requires that Provider and a user be loosely time-synchronized \cite{perrig2005timed}.
...
It is agnostic to the ECDSA and TESLA scheme parameters; therefore, it requires no changes in the event of cryptographic primitive breakage, it scales well with increases to the security level requirements and is flexible and future-proof to anticipated feedback from the Providers and SBAS stakeholders.
...
The Provider then derives a $2n$-bit integer from the private key and distributes to receivers as a public key.
...
Since the Hash Function is cryptographically secure, there is no {\em known} efficient algorithm that can compute the input Hash Point to the Hash Function that yields a specific output Hash Point.
...
The Provider broadcasts the last Hash Point along the Hash Path (herein, the "Hash Path End") together with an ECDSA signature.
...
Each time Providers release a Hash Point, Provider moves backward one Hash Point along its secret Hash Path to generate new HMACs.
