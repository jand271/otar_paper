...
The metadata associated with a authenticating ciphertext includes the expiration time.
...
If we send one large HMAC and any of the previous five messages are lost, then none of the messages can be authenticated because one needs the complete set of data and signatures to verify with HMAC.
Rather, the set of five smaller HMACs individually authenticate the five messages to protect against any message loss.
The probability that an adversary can forge an HMAC will be $2^{-16}$ since each HMAC is 16 bits.
To provide adequate protection against forgery, we specify a conservative forgery detection level.
If any one of the smaller HMACs returns not verified from the HMAC verification algorithm, then the receiver must throw out all non-MT51 information from that particular SBAS satellite and start over as if in receiver cold start.
While $2^{-16}$ is relatively weak security level, this is sufficient for the SBAS context because (1) an adversary does not yet have access to the delay-released key nor an HMAC verification oracle (from the cryptography security context), and (2) any forgery detection immediately throws all SBAS data.
To forge a useful navigation solution, an adversary must forge approximately seven SBAS messages undetected, and continue to make forgeries as SBAS navigation information is repeatedly broadcast.
...
The issue of data must be placed in the unused sections of the metadata, and the ciphertext page field would determine the relevant eight, in order, among the set of 92.
Since the issues of data requires only two bits, SBAS could use the unused German Key Hash, German Key Expiration, and Authenticating Key Hash to send 12 HMACs per MT51.
Alternatively, one could define a new message type that delivers 13 HMACs, the issue of data, and page number.
