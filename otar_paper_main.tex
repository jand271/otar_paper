...
This work addresses the full authentication scheme design, including receiver hardware requirements, key updates, and scheme maintenance, and uses a full-stack Monte-Carlo SBAS simulation to test and evaluate the scheme's performance.
Moreover, this work also addresses the time-synchronization and receiver hardware requirements to deter replay attacks.
...
Naturally, appending cryptographic data to the SBAS data serves as the best way to authenticate the SBAS data for civil users.
...
Wherever we specify ECDSA, one could use another asymmetric cryptographic algorithm (e.g., EC-Schnorr); however, we will specify ECDSA without losing generality for concreteness.
...
TESLA uses a delayed-release mechanism to authenticated data with less ciphertext than that required by ECDSA; however, it requires that Provider and a user be loosely time-synchronized \cite{perrig2005timed}.
...
It suggests that SBAS sends this MT every six messages and that this MT delivers 190-bits of TESLA authentication data and leaves 26-bits for the scheme's maintenance.
...
The protocol specifies a signature function and a verification function.
The Provider generates a secure random $n$-bit integer for long-term use as a secret private key.
The Provider then derives a $2n$-bit integer from the private key for the receiver as a public key.
Since the protocol is secure, there is no {\em known} efficient algorithm that can compute the private key given the public key.
By efficient algorithm, we mean any algorithm that is faster than exhaustive search on the total n-bit key space.
The Provider uses the signature function to compute a message's signature with the secret private key and shares the message and signature with the receiver.
The signature is a $4n$-bit integer.
The receiver uses the verification function with the public key to assert that the private key holder generated the message and signature.
Since the protocol is secure, there is no {\em known} efficient algorithm that can derive a public key's message signature without the associated private key.
...
Since the Hash Function is cryptographically secure, there is no {\em known} efficient algorithm that can compute the input Hash Point (also called the ``preimage'') to the Hash Function that yields a specific output Hash Point.
In other words, it is trivially easy to compute the output Hash Point of the Hash Function given an input Hash Point preimage, but one can only use exhaustive search to compute the input Hash Point preimage that corresponds to an output Hash Point.
It is a one-way path.
The domain of 128-bit integers, together with a randomized 128-bit salt inclusion to the Hash Function, make precomputation attacks (also called Rainbow Table Attacks) infeasible with modern supercomputers.
...
The Provider broadcasts the last Hash Point along the Hash Path (the "Hash Path End") together with an ECDSA signature.
Receivers assert the Hash Path End as authenticated with the ECDSA signature.
Provider uses the secret preimage Hash Point to the Hash Path End as a key to send authentication signatures along with the standard message set.
...
Each time the Provider release a Hash Point, the provider moves backward one Hash Point along its secret Hash Path to generate new HMACs.
...
The Provider releases that key the known schedule provided by the loosely-time-synchronized assumption, and the Provider notes that only messages received with the correct signature {\em before} that key release can be authenticated.
...
Moreover, the Provider and receiver must be loosely time-synchronized, which poses a significant Catch-22 problem since GNSS and SBAS Providers provide the time.
...
We propose a two-level ECDSA key structure.
...
The Provider will use its level-2 keys to authenticate TESLA Hash Path Ends, and keys derived from the TESLA Hash Paths will be used to authenticate the bulk of SBAS messages.
For all levels, the ciphertext delivered will accompany data (e.g., SBAS message preamble, MT, other data) that must be sent per the definitions below.
...
Each Hash Point, except the Hash Path End, will be associated with five keys that will authenticate five messages with HMAC.
...
The key $k_j$ for each message $m_j$ will be according to Equation \ref{eq: key}, and the signature $s_j$ thereof will be according to Equation \eqref{eq: signature}.
$t_j$ is the integer time of the authenticated messages broadcast and receipt.
We discuss the necessity of the xor operations of Equation \eqref{eq: key} in Section \ref{sec: time and keys}.
\begin{align} 
...
Each MT50 will authenticate the previous five messages using HMAC and the Hash Points sent at the {\em following} MT50 according to Equation \eqref{eq: hash path} along the current precomputed Hash Path.
...
Regarding ciphertext efficiency, the overhead required to OTAR a Hash Path does not increase with Hash Path length.
...
If Provider finds the overhead required to OTAR a Hash Path too burdensome on the schedule, Provider can double the Hash Path's length to halve Hash Path OTAR's frequency.
...
This ensures that if a single message is lost, the other four messages' authentication remains unaffected; however, the short 16-bit HMAC length relies on a short time between authentication.
...
The design of MT51 serves only to deliver ciphertext so that its use can be flexible and recycled among different OTAR designs and applications that require delivery of psuedorandom data such as keys, signatures, and Hash Points. 
Its purpose is to deliver large chunks of ciphertext to maintain the SBAS authentication scheme and the associated metadata on how the receiver should interpret that ciphertext.
...
We select 128 and 256-bit security level ECDSA keys since NIST provides standardized, secure elliptic curves of those security levels.
...
If these features are not desired, one could add redundant HMACS to the MT51 message to redundantly authenticate messages for the event of an MT50 loss.

\subsubsection{Fountain Codes}

The previous section discussed a breadth of options for the meta data; however, depending on the Provider's preferences on which features to incorporate into the meta data, there will be spare bits.
In the minimum case, where the Provider does not allow parallel key management and fixed expiration times, there could be around 64 spare bits of meta data.
Whatever the case may be, we suggest using the spare bits for Fountain Codes to (1) increase the reliability of delivering MT51 messages and (2) decrease the number of transmitted messages required for an authenticated first fix.
...
Furthermore, the next keys, applicable immediately upon expiration of the current keys but be sent well before their actual use for a seamless transition as keys expire.
To protect a specific new key's security, it would be prudent to only send that new key just before use.
For instance, we suggest the Provider send the next 100-week level-1 key repeatedly beginning five weeks before its actual use, send the next 10-week level-2 and 1-week TESLA Hash Path End 1 week before its actual use.
...
To reincorporate these two features, Provider could use spare metadata from MT51 to distribute the salt instead.
...
To break the TESLA protocol's security, an attacker must hack the receiver time so that its time is 6 seconds behind the Provider time.
