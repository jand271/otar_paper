...
Instead, the set of five smaller HMACs individually authenticate each of the five messages individually accommodating message loss.
The probability that an adversary can forge an HMAC will be $2^{-16}$ since each HMAC is 16 bits.
To provide adequate protection against forgery, we specify a conservative forgery detection approach.
If any one of the smaller HMACs fails the HMAC verification algorithm, then the receiver must throw out all non-MT51 information from that particular SBAS satellite and start over collecting new SBAS data.
