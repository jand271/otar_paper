...
We propose the Hash Path be defined according to Equation \eqref{eq: hash path} with $||$ denoting concatenation and $\oslash$ integer division.
\begin{equation}\label{eq: hash path}
p^P_{i+1} = \textrm{H} \left(p^P_i || S^P || (t_i \oslash 6) \right)
\end{equation}
...
We discuss the necessity of the concatenation and HMAC operations of Equation \eqref{eq: key} in Section \ref{sec: HMAC security}.
\begin{align} 
...
\subsection{Cryptographically Independent Keys and HMAC Security} \label{sec: HMAC security}

For our small HMAC use in this scheme to be secure, we must assert there exists no known efficient algorithm that can generate HMAC forgeries under our use case.
