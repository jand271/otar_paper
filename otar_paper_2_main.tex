...
\section{TESLA Design Methodology} \label{sec:tesla_design_methodology}
	...
	\subsection{TESLA Loss Tolerance} \label{sub:tesla_loss_tolerance}
		...
		TESLA allows receivers to derive past Hash Points since they derives from later broadcast Hash Points released backward along Provider's precomputed Hash Path held secret.
		...
		The smaller 16-bit HMAC design of MT50 aids in general message tolerance.
		Suppose instead, each MT50 contained a single HMAC that authenticated the previous five messages together.
		If any of the five previous messages were lost, then the receiver would not be able to verify any of the five messages.
		Therefore, we specify each HMAC from the set of five smaller HMACs individually authenticates each of the five prior messages individually thus accommodating message loss.
	...
	\subsection{TESLA Efficiency} \label{sub:tesla_efficiency}
		...
		The authentication ciphertext bits required to OTAR a Hash Path does not increase with Hash Path length.
		...
		Therefore, the OTAR transmission frequency is mostly driven by the desired time to first fix, see Section \ref{sub:mt51_schedule_frequency}.

		To meet a standard 128-bit security level for TESLA, we must use cryptographic independent 128-bit long keys for reach HMAC.
		...
		This is secure provided each of the keys derived are cryptographically independent which is achieved by the intermediate HMAC operation of Equation \eqref{eq: key}, see Section \ref{sub:tesla_security}.
		...
		\subsubsection{TESLA Hash Path Counter from Time} \label{subsub:time_counter}
			...
			If Provider were to arbitrarily switch Hash Paths, there is a constant-time complexity for the receiver to check if a new Hash Point hashes to the current Hash Path or a new one, as seen in Algorithm \ref{alg: TESLA receiver}.
	...
	\subsection{TESLA Security} \label{sub:tesla_security}
		...
		This probability is sufficiently low that, even with the support of vast computational resources, it should not be expected to ever occur.
		...
		To do this securely, we must ensure that each piece of information, each authenticated with an HMAC, must have its own cryptographically independent HMAC key.
		...
		The PRN code allows for all of the satellites to share the same Hash Path, since each satellite has its own unique PRN code.
		Our choices of time and PRN code is arbitrary, except that our selection necessarily guarantees uniqueness: each message from each satellite gets a cryptographically independent key to derive the HMAC.
...
\section{OTAR Design Methodology} \label{sec:otar_design_methodology}
	...
	\subsection{Metadata Design} \label{sub:metadata_design}
		...
		Since the payload is pseudorandom, there must include metadata that allows the receiver associate the unique MT51 payloads.
	...
	\subsection{Core Constellation Ephemerides Authentication} \label{sub:core_eph}
		...
		Table \ref{tab: meta-data} already provides the required metadata designations: (1) the germane core constellation system, (2) the cipher text type, and (3) the authenticating ciphertext page number.
		...
		Second, the HMACs must be signed with appropriate TESLA hash path key $k_j$ at $t_j$ of sending, the SBAS PRN and the satellite vehicle number, each from Equations \eqref{eq: ephemerides key} and \eqref{eq: ephemerides signature}.
		Like as described in Section \ref{sub:tesla_security}, the HMAC keys of Equation \eqref{eq: ephemerides signature} must be cryptgraphically independent, necessitating Equation \eqref{eq: ephemerides key}.
...
\section{Regarding a New SBAS System} \label{sec:regarding_a_new_sbas_system}
	...
	For the scheme of this work, special care was taken to ensure backwards compatibility with existing receivers.
	This is achieved by having the authentication occur via appending two messages, MT50 and MT51, to the schedule.
	Old receivers need only disregard MT50s and MT51s.
	However, how would this scheme change if we did not need to manage backwards compatibility?
	Since the CRC is redundant with the forward error correction SBAS features, We would suggest that that the 24-bit CRC be replaced with a 24-bit HMAC.
	This would eliminate the need to MT50.
	Instead, MT51 could distribute the Hash Points every sixth message, in addition to the OTAR Authentication Stack in pieces every 1 in 18 messages.
...
\section{Conclusion} \label{sec:conclusion}
	...
	Given that we can achieve authentication with just two MTs on the I-channel, we believe that this scheme, or something similar, to be the best way forward to achieve SBAS authentication.
