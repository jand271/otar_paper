...
\begin{abstract}
	Herein we delineate a complete SBAS authentication scheme, including over-the-air re-keying (``OTAR'') that uses Elliptic Curve Digital Signature Algorithm (``ECDSA'') and Timed Efficient Stream Loss Tolerant Authentication (``TESLA'') without using the Quadrature (``Q'') channel.
...
\section{Introduction} \label{sec:introduction}
	...
	Moreover, this work leverages prior art build specific features to the TESLA scheme to assert security efficiently \cite{chain-security} and allows additional features relevant to SBAS stakeholders.
	...
	The tool shows that our proposed design outperforms Key Performance Indicators (each a ``KPI''), such as time to first authenticated fix, time to authentication per message, and robustness to message loss.
...
\section{SBAS Authentication Scheme Definition} \label{sec:sbas_authentication_scheme_definition}
	...
	\subsection{TESLA Hash Path and Keys} \label{sub:tesla_hash_path}
		...
		\begin{align}
			k_j &= \textrm{HMAC}(p^P_i, t_j || \textrm{PRN} \label{eq: key}) \\
	...
	\subsection{MT51} \label{sub:mt51}
		...
		Table \ref{tab: mt51} and Table \ref{tab: meta-data} together provide our proposed definition of MT51 with bit allocations.
		Provider must broadcast MT51 messages approximately every 1 in 18 messages, see Section \ref{sub:mt51_schedule_frequency}.
		...
		\begin{table}%\label{tab: mt51}
			\center
			\begin{tabular}{|c|c|c|c|c|} \hline
				Preamble & MT & Cipher Text Metadata & Cipher Text & CRC \\ \hline
				4 & 6 & 88 & 128 & 24 \\ \hline
			\end{tabular}
			\caption{Bit allocation proposed for MT 51 at 250 bits per message. Table \ref{tab: meta-data} providers greater detail of the ciphertext metadata.}
			\label{tab: mt51}
		\end{table}
	...
	\subsection{Procedures} \label{sub:procedures}
		...
		In addition, Provides must assemble OTAR data by coordinating with CA according to Section \ref{sub:ecdsa_key_structure}.
		...
		A receiver should not use any authenticated data.
		...
		Upon complete receipt of the Authentication Stack, receivers must hash the current Hash Point from the most recent MT50 to the MT51-provided Hash Path End.
		...
		\begin{algorithm}%\label{alg: TESLA provider}
			...
			\For{$i = [n, n-1, \cdots, 2, 1]$}{
				...
				Derive $k_1 \cdots k_5$ from Equation \eqref{eq: key} from $p_i$ and the sending time of $m_1 \cdots m_5$. \\
				Derive $s_1 \cdots s_5$ from Equation \eqref{eq: signature} from $m_1 \cdots m_5$ and $k_1 \cdots k_5$. \\
				Recall the correct release $p_r$: \\
				\eIf{$i == n$}{
					$p_r = p^{P-1}_1$
				}{
					$p_r = p^P_{i+1}$
		...
		\begin{algorithm}%\label{alg: TESLA receiver}
			\caption{Receiver Procedures for Authenticated message Distribution with TESLA Hash Paths.}
			For all MT51 messages. \\
			\label{alg: TESLA receiver}
			\While{on}{
				Receive MT51 data pages associated via the MT51 metadata and store into three hash tables $H_1$, $H_2$, and $H_3$. \\
				...
				Await receipt of all needed pages of MT51, called the Authentication Stack, to assert authenticated Hash Path Ends. \\
				A Hash Path End element $p^P_{n+1}$ stored in $H_3$ is called authenticated if itself and signature are ECDSA verified by an authenticated element in $H_2$. \\
				A public ECDSA key element stored in $H_2$ is called authenticated if itself and signature data are ECDSA verified by an authenticated element in $H_1$. \\
				A public ECDSA key element stored in $H_1$ is called authenticated if it was prestored from the CA.
			}

			For all non-MT51 messages \\
			\While{on}{
				Receive and cache unauthenticated non MT50 messages $m_7 \cdots m_{11}$ \\
				\If{$m_7 \cdots m_{11}$ are MT51}{
					Follow the MT51 procedures above. \\
				}
				Receive and cache MT50 message $m_{12}$ with signatures $s_7 \cdots s_{11}$ and Hash Point $p_r$ \\
				Recall $m_1 \cdots m_5$ \\ 
				Recall $m_6$ with signatures $s_1 \cdots s_5$ \\
				$i \gets 1$ \\
				$p_t \gets H(p_r)$ via Equation \eqref{eq: hash path} \\
				\While{$i \leq$ Maximum Iteration}{
					\eIf{$p_t \in H_3$ and $H_3(p_t)$ is authenticated and not expired via ECDSA through level-1}{
						$m_{12}$ is authenticated \\
						Check $m_1 \cdots m_5$ with $p_r$ and $s_1 \cdots s_5$ to authenticate $m_1 \cdots m_5$ \\
					}{
						$p_t \gets H(p_t)$
					}
					$i \gets i + 1$ \\
					Note: caching previously authenticated $p_t$ prevents excessive hashing to Hash Path End each iteration.
				}
...
\section{TESLA Design Methodology} \label{sec:tesla_design_methodology}
	...
	\subsection{TESLA Loss Tolerance} \label{sub:tesla_loss_tolerance}
		...
		TESLA allows receivers to derive past Hash Points from later broadcast Hash Points released backward along Provider's precomputed Hash Path held secret.
	...
	\subsection{TESLA Efficiency} \label{sub:tesla_efficiency}
		...
		Increasing the Hash Path length requires Provider to compute a longer Hash Path per OTAR in the event of cold start, but this burden is negligible as commodity hardware can compute Hash Paths on the lengths specified in this work in seconds, see Section \ref{sub:procedures}.
		At a 128-bit Hash Point length, a Hash Path length on the order of decades is safe from attack \cite{Neish_Dissertation}.
		Therefore, the OTAR transmission frequency is mainly driven by the desired time to first authenticated fix, see Section \ref{sub:mt51_schedule_frequency}.

		To meet a standard 128-bit security level for TESLA, we must use cryptographic independent 128-bit long keys for each HMAC.
		...
		Specifically, each key from Equation \ref{eq: key}, both in time and by satellite, derive from the same 128-bit Hash Point distribution, thereby not needing a separate Hash Path for each stream of authenticated information.
		...
		WAAS could use the same Hash Path for all of its satellites and broadcast the set of unique MT51 messages that make up the Authentication Stack out of phase, decreasing the time-of-first-authenticated-fix by 66.6\%, while accommodating receivers that do not track each satellite.
		Our selection for a single geostationary SBAS satellite time of first fix is discussed in Section \ref{sub:mt51_schedule_frequency}.
		...
		\subsubsection{TESLA Hash Path Counter from Time} \label{subsub:time_counter}
			...
			To accommodate perturbations of the authentication schedule, we propose (1) nominally sending each MT50s when $t_i \mod 6 = 0$ and (2) performing an integer division of 6 on time $t_i$.
			...
			In Equations \eqref{eq: key} and \eqref{eq: hash path}, we proposed including the times $t_i$ and $t_j$ in the TESLA counter and Hash Point to key derivations.
	...
	\subsection{TESLA Security} \label{sub:tesla_security}
		...
		To do this securely, we must ensure that each piece of information authenticated with an HMAC must have its own cryptographically independent HMAC key.
		...
		In the specific case of Equation \eqref{eq: key}, the HMAC message field includes the message time and the satellite PRN code.
		...
		The PRN code allows all of the satellites to share the same Hash Path since each satellite has its own unique PRN code.
		Our time and PRN code choices are arbitrary, except that our selection guarantees uniqueness: each message from each satellite gets a cryptographically independent key to derive the HMAC.
		...
		An adversary could listen for any SBAS message and then broadcast that message four additional times before the next MT50 or forge that message as coming from another satellite.
		Hence, to assert authenticating security of a particular message from a particular satellite, the HMAC key must derive from the time and source.
		...
		For example, if any of the smaller HMACs fails the HMAC verification algorithm, the receiver must throw out all non-MT51 information from that particular SBAS satellite and start over collecting new SBAS data.
...
\section{OTAR Design Methodology} \label{sec:otar_design_methodology}
	...
	The modular design also allows easy expansion of additional features described in Section \ref{sub:metadata_design} that may desirable to SBAS stakeholders.
		\begin{table}[H]
		\center
		\begin{tabular}{|c|c|c|c|c|} \hline
	...
	\subsection{Metadata Design} \label{sub:metadata_design}
		...
		Since the payload is pseudorandom, metadata must exist so that the receiver to associate the unique MT51 payloads.
		...
		If EGNOS broadcasts the WAAS keys once an hour, any westward aircraft will receive the necessary WAAS keys to operate before reaching North America, decreasing the WAAS time of the first authenticated fix to zero. 
	...
	\subsection{Core Constellation Ephemerides Authentication} \label{sub:core_eph}
		...
		Each 16-bit HMAC within the MT51 payload corresponds to a specific satellite's broadcast ephemerides, and the MT51 metadata informs the receiver of which satellites correspond to the payload HMACs.
		...
		\begin{align}
			k_j &= \textrm{HMAC}(p^P_i, t_j || \textrm{PRN}_\textrm{SBAS} || \textrm{SVN} \label{eq: ephemerides key}) \\
		...
		Equations \eqref{eq: ephemerides key} and \eqref{eq: ephemerides signature} use the sending time $t_j$, the SBAS PRN, and the core constellation satellite vehicle number.
	...
	\subsection{MT51 Schedule Frequency} \label{sub:mt51_schedule_frequency}
		...
		However,  any choice must meet minimum security requirements connected to the required computational time to guess the keys exhaustively.
...
\section{Regarding a New SBAS System} \label{sec:regarding_a_new_sbas_system}

	For the scheme of this work, we ensure backward compatibility with existing receivers by achieving authentication via appending two messages, MT50 and MT51, to the SBAS schedule.
	Old receivers need only disregard MT50s and MT51s.
	However, how would this scheme change if we did not need to manage backward compatibility?
	We suggest a 24-bit HMAC replace the current 24-bit CRC.

	SBAS already provides forward error correction features to the signal.
	Therefore the current use of the 24-bit CRC is redundant.
	Using these 24-bits for authentication provides several advantages.
	First, 24-bit HMACs provide better security than 16-bit HMACs.
	Second, appending the HMAC to its message aids with efficiency and loss tolerance.
	As written now, this scheme separately sends messages and HMACs.
	If a message or its succeeding MT50 are lost, both messages are effectively lost.
	The message and the HMAC must be received to authenticate, but the intermittent loss of each affects the other.
	It would be more efficient to place the message and the HMAC together in the same message so that a particular message lost does not affect other messages' bit efficiency and authenticity. 
	Third, replacing the CRC with the 24-bit HMAC would eliminate the need to MT50.
	Instead, after augmenting the MT51 metadata, MT51 could distribute the Hash Points every sixth message, in addition to the OTAR Authentication Stack in pieces every 1 in 18 messages.
...
\section{Full Stack Simulation AND KPIs} \label{sec:full_stack_simulation}
	...
	The time-to-first-fix is less than our requirement from Section \ref{sub:mt51_schedule_frequency} because the simulated Provider sent the 16 unique messages within open slots of the SBAS schedule.
