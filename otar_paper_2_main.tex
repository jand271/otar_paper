...
\section{SBAS Authentication Scheme Definition} \label{sec:sbas_authentication_scheme_definition}
	...
	\subsection{TESLA Hash Path and Keys} \label{sub:tesla_hash_path}
		...
		Each Hash Point, except the Hash Path End, will be associated with at least five keys that will authenticate at least five messages with HMAC, depending on the number of iterations of Equation \eqref{eq: key}.
	...
	\subsection{Procedures} \label{sub:procedures}
		...
		To augment those algorithms to handle the multiple satellite or multiple frequency case, more than five messages should be signed or verified for each for-loop operation.
...
\section{TESLA Design Methodology} \label{sec:tesla_design_methodology}
	...
	\subsection{TESLA Efficiency} \label{sub:tesla_efficiency}
		...
		Increasing the Hash Path length requires Provider and receivers to compute a longer Hash Path per OTAR in the event of cold start, but this burden is negligible as commodity hardware can compute Hash Paths on the lengths specified in this work in seconds, see Section \ref{sub:procedures}.
		...
		Specifically, each key from Equation \eqref{eq: key}, both in time and by satellite, derive from the same 128-bit Hash Point distribution, thereby not needing a separate Hash Path for each stream of authenticated information.
		...
		We can extend this argument for the L1 and L5 frequency bands, decreasing the time-of-first-authenticated-fix by 83.3\%.
		Our selection of a time-of-first-authenticated-fix for a receiver tracking a single geostationary SBAS satellite on a single frequency is discussed in Section \ref{sub:mt51_schedule_frequency}.
		...
		\subsubsection{ECDSA-derived Hash Path Salt} \label{subsub:ecdsa_salt}
			...
			\begin{algorithm}%\label{alg: salt}
				...
				Provider generates ECDSA signature for $p^P_{n+1}$ with $C$ and broadcasts before actual use of Hath Path; \\
				{\bf Receiver} \\
		...
		\subsubsection{TESLA Hash Path Counter from Time} \label{subsub:time_counter}
			...
			Using time-based counter aids with Security.
	...
	\subsection{TESLA Security} \label{sub:tesla_security}
		...
		Hence, to assert authenticating security of a particular message from a particular satellite, the HMAC key must derive from the time, source, source, and frequency.
...
\section{OTAR Design Methodology} \label{sec:otar_design_methodology}
	...
	\subsection{Metadata Design} \label{sub:metadata_design}
		...
		Without this feature, when an aircraft enters a new SBAS service volume the first time of the week (or 10 or 100 weeks), it must act as a cold-start receiver liable to the amount of time required to get the Authentication Stack.
	...
	\subsection{MT51 Schedule Frequency} \label{sub:mt51_schedule_frequency}
		...
		With multiple geostationary satellites and frequencies, the satellites can broadcast the unique MT51s out of phase to decrease the authenticated first fix time.
