...
\section{Introduction} \label{sec:introduction}
	...
	This work builds and an expands upon our work presented in \cite{Anderson2021}, including updating security details, updating definitions based upon some SBAS Stakeholder feedback, and including additional results.
	...
	Some prior art has suggested using the Quadrature (``Q'') channel to deliver the authenticating psuedorandom data \citep{todd-pki-scheme, other_schemes}; however, those solutions would require power from the current use of the In-phase (``I'') channel.
	...
	Some prior art has suggested using a combination of ECDSA with another algorithm called Timed Efficient Stream Loss Tolerant Authentication (``TESLA'') \citep{Neish_Dissertation, gal-os-tesla} since it provides more efficient use of authenticating psuedorandom data and is loss-tolerant.
	TESLA uses a delayed-release mechanism to authenticate data with less authenticating psuedorandom data than that required by ECDSA; however, it requires that Provider and a user be loosely time-synchronized \citep{perrig2005timed}.
	...
	Moreover, this work leverages specific features of TESLA scheme to assert security efficiently \citep{chain-security} and allows additional features relevant to SBAS stakeholders.

	Another challenge lies with receiver computational considerations.
	Some prior art has explored how TESLA and ECDSA computations would fare with use in the GNSS, SBAS, and smartphone context \citep{tesla-cpus}.
	...
	Therefore, we expect that if these methods burden current receivers, manufacturers could augment their chips at a minimal cost to maintain desirable computational loads.

	Prior art suggested appending a single message type (``MT'') to SBAS for authentication and its maintenance\citep{Neish_Dissertation}.
	...
	To evaluate the proposed method against prior art, we implement a full-stack SBAS simulation of our design by augmenting an existing SBAS simulation tool called Matlab Algorithm Availability Simulation Tool (``MAAST'')\citep{MAAST}.
	...
	There are ways from the prior art by which receivers can establish trust in GNSS ranging signals\citep{Psiaki2016,Fernandez-Hernandez2018}; however, GNSS ranging signals are not yet rigorously authenticated with cryptography.
	Some prior art has described how an onboard receiver clock could be used to detect such an attack, given clock uncertainty\citep{time_sync_paper}.
	...
	\subsection{ECDSA} \label{sub:ecdsa}
		...
		There are details not mentioned in the following summary description of ECDSA, so for a more precise definition, we refer the reader to widely available detailed definitions in cryptographic literature or the internet~\citep{boneh2017graduate}.
...
\section{SBAS Authentication Scheme Definition} \label{sec:sbas_authentication_scheme_definition}
	...
	\subsection{ECDSA Key Structure} \label{sub:ecdsa_key_structure}
		...
		We propose a two-level ECDSA key structure, as suggested \citep{Neish_Dissertation}.
	...
	\subsection{MT50} \label{sub:mt50}
		...
		Provider should set the cadence of integrity messages to always immediately preceed the scheduled MT50s to minimize integrity message authentication time.
		If the receiver is not able to authenticate a message because of a lost MT50, it generally disregards that message (information that alerts the receivers to decrease the level of trust need not be disregarded).
	...
	\subsection{MT51} \label{sub:mt51}
		...
		seventh, the segment number of the authenticating psuedorandom data so that the receiver can aggregate the authenticating psuedorandom data segments over time.
			...
			\begin{algorithmic}
				...
				\While{on}
					...
					\While{$i \leq$ Maximum Iteration From Max Hash Path Length}
						...
						\EndIf
						\State $i \gets i + 1$
						\State Note: caching previously authenticated $p_t$ prevents excessive hashing to Hash Path End each iteration. There is no need to hash down to the Hash Path End more than once.
					\EndWhile
...
\section{TESLA Design Methodology} \label{sec:tesla_design_methodology}
	...
	\subsection{TESLA Loss Tolerance} \label{sub:tesla_loss_tolerance}
		...
		Regarding loss tolerance of messages and the security of 16-bit HMACs, we implement the key idea of \citep{Neish_Dissertation}.
	...
	\subsection{TESLA Efficiency} \label{sub:tesla_efficiency}
		...
		At a 128-bit Hash Point length, a Hash Path length on the order of decades is safe from attack \citep{Neish_Dissertation}.
		...
		\subsubsection{ECDSA-derived Hash Path Salt} \label{subsub:ecdsa_salt}
			...
			Unsalted Hash Paths are susceptible to precomputation attacks, also known as Rainbow Table Attacks~\citep{boneh2017graduate}.
			...
			With Algorithm~\ref{alg: salt}, the salt derives from a public quantity derived from a nonce.
			For ECDSA, this is the curve point $C$ (the analogous number in EC-Schnorr signatures is usually called $r$).
			This $C$ is cryptographically secure random since it derives from the cryptographically-secure nonce generated at signing; using it as the Hash Path salt saves an MT51 message without compromising the signing key or the Hash Path.
		...
		\subsubsection{TESLA Hash Path Counter from Time} \label{subsub:time_counter}
			...
			Moreover, it allows the switching of Hash Paths to occur on a non-rigid schedule, which aids with security as well \citep{chain-security}.
			...
			This makes SBAS message spoofing more complex since any SBAS spoofer must also spoof the GNSS time.
			...
			This is acceptable because those messages tell receivers not to use the service, and the surrounding integrity messages will be authenticated anyway.
			Without this requirement, messages of substance will not be authenticated.

	\subsection{TESLA Security} \label{sub:tesla_security}

		Previous work determined the appropriate security-level lengths under conservative adversary models\citep{Neish_Dissertation}.
...
\section{OTAR Design Methodology} \label{sec:otar_design_methodology}
	...
	\subsection{MT51 Metadata Design} \label{sub:metadata_design}
		...
		Moreover, the delivery of 128-bits of authenticating psuedorandom data could be augmented with fountain codes \citep{gnss_fountain_codes}.
		...
		An 192-bit OTAR Payload Segment length almost maintains the integer-division property.
		...
		Alternatively, as features are removed, rather than shifting bit allocations from the metadata to OTAR Payload Segment, SBAS Providers could instead replace metadata bits with fountain codes arbitrarily.
	...
	\subsection{Core Constellation Ephemerides Authentication} \label{sub:core_eph}
		...
		\begin{table}
			...
			\end{tabular}
			\caption{Bit allocation for a particular MT 51 to authenticate a subset of ephemerides with a 128-bit payload.}
			\label{tab: nma}
		\end{table}
...
\section{TESLA Time Synchronization} \label{sec:tesla_time_synchronization}
	...
	The prior art has explored how to use clock tolerances to generate and use tolerance bounds to assert clock trust\citep{time_sync_paper}.
	...
	The original TESLA protocol provides a secure time-synchronization procedure: Provider would establish an internet server to accept and operate on receiver-generated nonces that would work within the secure Hash Path\citep{perrig2005timed}.
...
\section{Full Stack Simulation AND KPIs} \label{sec:full_stack_simulation}
	...
	For this work, MAAST implemented the Authenticate-Then-Use (``ATU'') framework for the receiver\citep{walter2021sbas}.
	...
	Therefore, among our simulations, TFAF occurred within two minutes, rather than the required five minutes.
...
\section{Conclusion} \label{sec:conclusion}
	...
	The scheme relies on three levels of security: (1) 128-bit-security TESLA, (2) 128-bit-security ECDSA, and (3) 256-bit-security ECDSA. 
	...
	The flexibility of the scheme derives from this work's suggestion to craft a single message for all cryptographic maintenance.
