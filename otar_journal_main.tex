...
\section{SBAS Authentication Scheme Definition} \label{sec:sbas_authentication_scheme_definition}
	...
	\subsection{MT50} \label{sub:mt50}
		...
		Even with an MT50 delay, each MT50 must sign the messages it would have signed without an alert, see Section~\ref{sub:subsub:time_counter}.
	...
	\subsection{Procedures} \label{sub:procedures}
		...
		\begin{algorithm}%\label{alg: TESLA receiver}
			...
			\While{on}{
				Receive MT51 payload segments associated via the MT51 metadata and store into three hash tables $H_1$, $H_2$, and $H_3$. 
				...
				Await receipt of all needed unique MT51 OTAR Payload Segments, called the Authentication Stack, to assert authenticated Hash Path Ends.
...
\section{TESLA Design Methodology} \label{sec:tesla_design_methodology}
	...
	\subsection{TESLA Efficiency} \label{sub:tesla_efficiency}
		...
		\subsubsection{TESLA Hash Path Counter from Time} \label{subsub:time_counter}
			...
			Both Option 1 or Option 2 provides the same security by increasing the HMAC-to-Hash-Point spread back above six seconds.
...
\section{OTAR Design Methodology} \label{sec:otar_design_methodology}
	...
	\subsection{MT51 Metadata Design} \label{sub:metadata_design}
		...
		Therefore, we expect the results of Section~\ref{} to be repeatable with a design with a larger 128-bit OTAR Payload Segment.
...
\section{Full Stack Simulation AND KPIs} \label{sec:full_stack_simulation}
	...
	Within Figure~\ref{fig:results}, the availability map is slightly different, noticeable at the service volume boundaries over Alaska and Canada.
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.47\linewidth]{fig/no_auth_avail.png}
		\includegraphics[width=0.47\linewidth]{fig/auth_avail.png}
		\caption{
